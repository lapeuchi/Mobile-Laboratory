...
This is a simple asset pack that contains 118 prefabs from a mesh created by me. 

I have set up the prefabs in the Prefab folder for you to drag and drop into your scenes. 
Each Prefab is named EC(element number)_(element name) for easier searching. For example "EC1_Hydrogen"

There is a high res reflection probe set up in the example scene that might slow things down, so just disable it. I used it for the higher res promo video. 

Model
-----
The center cube of the model is 1m x 1m, so take that into consideration when placing the prefabs in your scene for rescaling.
...
118 Texture sets were made using a PBR workflow. 
