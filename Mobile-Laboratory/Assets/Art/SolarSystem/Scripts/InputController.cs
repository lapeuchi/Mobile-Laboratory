...
public class InputController : MonoBehaviour {
	...
	void ObjectInfo()
	{
		...
		if(raycast)
		{
			SSoundManager.instance.PlayHoverEffectSound(true);
			...
			{
				SUIManager.instance.SetHighlightedObjectName(objectHighlighted.name);
				SUIManager.instance.SetLabelNextToObject(objectHighlighted.name);

				// Switch to detailed camera view when clicked
				if(Input.GetMouseButtonUp(0))
				{
					SUIManager.instance.SwitchToDetailedPlanetView(selectedObject.name);
				}
			...
			{
				SUIManager.instance.SwitchToDetailedPlanetView(selectedObject.name);
				SUIManager.instance.DisablePlanetLabel();
			}
		}
		else
		{
			SSoundManager.instance.PlayHoverEffectSound(false);
